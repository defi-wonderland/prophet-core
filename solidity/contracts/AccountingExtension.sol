// SPDX-License-Identifier: AGPL-3.0-only
pragma solidity ^0.8.19;

import {IERC20} from '@openzeppelin/contracts/token/ERC20/IERC20.sol';
import {SafeERC20} from '@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol';

import {IWETH9} from '../interfaces/external/IWETH9.sol';

import {IAccountingExtension} from '../interfaces/IAccountingExtension.sol';
import {IOracle} from '../interfaces/IOracle.sol';

contract AccountingExtension is IAccountingExtension {
  using SafeERC20 for IERC20;

  IWETH9 public immutable WETH;

  // The available balance (ie deposit - amount currently bonded in a process), per depositor and per oracle
  mapping(address _bonder => mapping(IOracle _oracle => mapping(IERC20 _token => uint256 _balance))) public balanceOf;

  // The amount in a bond (request, dispute)
  mapping(address _bonder => mapping(IOracle _oracle => mapping(IERC20 _token => uint256 _amount))) public
    bondedAmountOf;

  constructor(IWETH9 _weth) {
    WETH = _weth;
  }

  // deposit transfers the funds from the caller into the contract and increases the caller’s virtual balance,
  // thus allowing them to take part in providing responses, voting or the bond escalation processes.
  // As the bonding module will be shared between all requests, the users will not have to constantly transfer
  // the funds in and out to interact with OpOO.
  function deposit(address _depositor, IOracle _oracle, IERC20 _token, uint256 _amount) external payable {
    // If ETH, wrap:
    if (msg.value != 0) {
      WETH.deposit{value: msg.value}();
      _token = IERC20(address(WETH));
      _amount = msg.value;
    } else {
      _token.safeTransferFrom(_depositor, address(this), _amount);
    }

    balanceOf[_depositor][_oracle][_token] += _amount;

    // TODO: add msg.sender to the event in case we want to fetch off-chain who used the Oracle as proxy?
    emit Deposit(_depositor, _token, _amount);
  }

  // withdraw returns the user’s funds, adding any payments for provided responses and subtracting the slashed amounts.
  function withdraw(address _depositor, IOracle _oracle, IERC20 _token, uint256 _amount) external {
    uint256 _balance = balanceOf[_depositor][_oracle][_token];

    if (_balance < _amount) revert AccountingExtension_InsufficientFunds();

    // We checked supra
    unchecked {
      balanceOf[_depositor][_oracle][_token] -= _amount;
    }

    // If weth, unwrap
    if (_token == IERC20(address(WETH))) {
      WETH.withdraw(_amount);
      payable(_depositor).transfer(_amount);
    } else {
      _token.safeTransfer(_depositor, _amount);
    }

    // TODO: add msg.sender to the event in case we want to fetch off-chain who used the Oracle as proxy?
    emit Withdraw(_depositor, _token, _amount);
  }

  // pay is the function by which the user's virtual balance amount is increased, often as a result of submitting correct responses, winning disputes, etc
  function pay(IERC20 _token, address _payee, address _payer, uint256 _amount) external {
    // pay is considered as coming from the oracle
    // NOTE: I'm assuming the payer pays with bonded balance
    // TODO: Can I assume the bonded amount will always be greater than the amount to pay?
    if (bondedAmountOf[_payer][IOracle(msg.sender)][_token] < _amount) revert AccountingExtension_InsufficientFunds();

    balanceOf[_payee][IOracle(msg.sender)][_token] += _amount;
    unchecked {
      bondedAmountOf[_payer][IOracle(msg.sender)][_token] -= _amount;
    }

    emit Pay(_payee, _payer, IOracle(msg.sender), _token, _amount);
  }

  // slash opposite of pay, decreases the user’s virtual balance as a result of malicious behavior.
  function slash(IERC20 _token, address _slashed, address _disputer, uint256 _amount) external {
    // This should never happen
    // NOTE: I'm assuming the slasher gets bonded balance slashed
    if (bondedAmountOf[_slashed][IOracle(msg.sender)][_token] < _amount) revert AccountingExtension_InsufficientFunds();

    balanceOf[_disputer][IOracle(msg.sender)][_token] += _amount;
    unchecked {
      bondedAmountOf[_slashed][IOracle(msg.sender)][_token] -= _amount;
    }

    emit Slash(_slashed, _disputer, IOracle(msg.sender), _token, _amount);
  }

  // Bond an amount for a request or a dispute
  function bond(address _bonder, IERC20 _token, uint256 _amount) external {
    // Initiated by the oracle
    if (balanceOf[_bonder][IOracle(msg.sender)][_token] < _amount) revert AccountingExtension_InsufficientFunds();

    unchecked {
      balanceOf[_bonder][IOracle(msg.sender)][_token] -= _amount;
      bondedAmountOf[_bonder][IOracle(msg.sender)][_token] += _amount;
    }

    emit Bond(_bonder, IOracle(msg.sender), _token, _amount);
  }

  // Unbond an amount after a request is finalised/dispute is resolved
  function release(address _bonder, IERC20 _token, uint256 _amount) external {
    // Initiated by the oracle too
    if (bondedAmountOf[_bonder][IOracle(msg.sender)][_token] < _amount) revert AccountingExtension_InsufficientFunds();

    unchecked {
      bondedAmountOf[_bonder][IOracle(msg.sender)][_token] -= _amount;
      balanceOf[_bonder][IOracle(msg.sender)][_token] += _amount;
    }

    emit Release(_bonder, IOracle(msg.sender), _token, _amount);
  }

  receive() external payable {}
}
